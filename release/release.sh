...
#
RELEASE=/soi/release
DISTRO=/soi/jpv-sun/build/distributions
RESOURCES=/soi/jpv-sun/core/src/main/resources
...
echo "SOI 6.$VERSION integrada el $(date)" > $CURRENT/etc/version
...
# Update version 
echo "SOI 6.$VERSION+ desarrollando a partir de $(date)" > $RESOURCES/version
if [ "$1/A" == "/A"  ]; then
    VERSION=$((VERSION+1))
